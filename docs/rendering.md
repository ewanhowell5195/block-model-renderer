...
| `options.release` | `false` | Drops `blockLight` and `skyLight`, and the texture's CPU copy once it's uploaded, as on [`optimizeScene`](optimization.md#releasing-cpu-copies). `lightAt` then throws |
