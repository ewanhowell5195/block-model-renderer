...
The option is accepted by every entry point ([`renderBlock`](api.md), [`renderItem`](api.md), [`renderModel`](api.md), [`parseBlockstate`](api.md), [`parseItemDefinition`](api.md), [`loadModel`](api.md)) and is also propagated onto model objects as `model.version`, so manually constructed models can carry it through too. Asset-level behaviors (like the trim palette locations) read the version from the prepared bundle: it sticks there from the first versioned call, or pass it up front with `prepareAssets(sources, { version })`.
