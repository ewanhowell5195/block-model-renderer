...
The bundled block entity overrides are fetched once as a single `assets.zip` resolved relative to the module URL (works on jsDelivr and through bundlers). If yours lives somewhere unusual, point at it with [`configure({ assetsUrl })`](api.md). A failed fetch isn't cached, and changing `assetsUrl` refetches; either applies to assets prepared afterwards, while already-prepared bundles keep the entries they were built with.
