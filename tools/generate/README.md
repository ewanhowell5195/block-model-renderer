...
| Data | Source |
|---|---|
| `defaultStates` | every block's real default state (`block.defaultBlockState()`), property by property. Compressed into `default_blockstates.json`'s `{ properties, blocks }` shape: one global value per property name (the one most blocks default to) plus a rule per group that disagrees, carrying only the properties where they differ. Scalars only, never the priority arrays the preferred overlay uses, so a lookup has exactly one answer; the cover is replayed over every block and property and must reproduce the game's default state exactly, falling back to exact id lists if a suffix cover ever wouldn't. The hand-written `default_blockstates_preferred.json` layers on top of it and is not generated |
